...
class Normal extends Distribution {
    ...
    function zScore(index) {
        return (Math.floor(index / 10) * .1) + (index % 10) * .01;
    }

    function zFromP(p: Float): Float {
        var m = p / 2;
    ...
    public function new(mu = 0.0, sigma = 1.0) {
        super();
        this.mu = mu;
        this.sigma = sigma;
    }

    public function mean(mu: Float): Normal {
        this.mu = mu;
        return this;
    }

    public function deviation(sigma: Float): Normal {
        this.sigma = sigma;
        return this;
    }
    ...
    override public function generate(): Float {
        ...
        var v = super.generate();
        var w = super.generate();
        
        var z1 = Math.sqrt(-2 * Math.log(v)) * Math.sin(2 * Math.PI * w);
        var z2 = Math.sqrt(-2 * Math.log(v)) * Math.cos(2 * Math.PI * w);
